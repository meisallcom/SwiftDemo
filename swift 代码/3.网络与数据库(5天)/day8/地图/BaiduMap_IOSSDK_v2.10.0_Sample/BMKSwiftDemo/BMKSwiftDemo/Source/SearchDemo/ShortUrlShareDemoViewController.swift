...
class ShortUrlShareDemoViewController: UIViewController, BMKMapViewDelegate, BMKShareURLSearchDelegate, BMKPoiSearchDelegate, BMKGeoCodeSearchDelegate, MFMessageComposeViewControllerDelegate  {
    ...
    @IBAction func poiShortUrlShare(sender: UIButton) {
    ...
    @IBAction func reverseGeoShortUrlShare(sender: UIButton) {
    ...
    ////路线规划短串分享url
    @IBAction func routePlanShortUrlShare(sender: UISegmentedControl) {
    ...
    */
     ...
     *@param error 错误号，@see BMKSearchErrorCode
     */
    func onGetRoutePlanShareURLResult(searcher: BMKShareURLSearch!, result: BMKShareURLResult!, errorCode error: BMKSearchErrorCode) {
        if error == BMK_SEARCH_NO_ERROR {
            shortURL = result.url
